...
fs.writeFileSync(dirs.srcPath + 'scss/style.styl', styleImports);
...
console.log("Режим сборки: ", process.env.NODE_ENV, " isDev: ", isDev);
...
gulp.task('style', function () {
  ...
  return gulp.src(dirs.srcPath + 'scss/style.styl')
    .pipe(plumber({
    ...
    .pipe(stylus({use:[nib()],includePaths: [__dirname+'/']}))
...
gulp.task('copy:img', function () {
  ...
  return gulp.src(lists.img)
    .pipe(newer(dirs.buildPath + '/images'))  // оставить в потоке только изменившиеся файлы
    .pipe(size({
      title: 'Размер',
      showFiles: true,
      showTotal: false,
...
gulp.task('copy:favicon', function () {
  ...
  return gulp.src(dirs.srcPath + '/favicon/*.{png,ico,svg}')
    .pipe(gulp.dest(dirs.buildPath + '/images/favicon'));
});
...
gulp.task('serve', gulp.series('build', function() {
  ...
  // Стили
  let stylePaths = [
    dirs.srcPath + 'scss/**/*.styl',
  ];
  for (let i = 0, len = lists.blocksDirs.length; i < len; ++i) {
    stylePaths.push(dirs.srcPath + lists.blocksDirs[i] + '*.styl');
  }
...
function getFilesList(config){
  ...
  for (let blockName in config.blocks) {
    ...
    if(fileExist(blockPath)) {
      ...
      // Стили
      if(fileExist(blockPath + blockName + '.styl')){
        res.css.push('../' + config.dirs.blocksDirName + '/' + blockName + '/' + blockName + '.styl');
        if(config.blocks[blockName].length) {
          config.blocks[blockName].forEach(function(elementName) {
            if(fileExist(blockPath + blockName + elementName + '.styl')){
              res.css.push('../' + config.dirs.blocksDirName + '/' + blockName + '/' + blockName + elementName + '.styl');
            }
          });
        }
      }
      else {
        console.log('---------- Блок ' + blockName + ' указан как используемый, но не имеет styl-файла.');
      }
